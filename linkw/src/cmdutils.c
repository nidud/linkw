...
bool ProcArgSPList( bool (*rtn)( void ), tokcontrol ctrl )
{
    do {
        if( !WildCard( rtn, ctrl ) ) {
            return( FALSE );
        }
    } while( GetToken( SEP_SPACE, ctrl ) );
    return( TRUE );
}
...
/*************************************************************/
{
    ...
    if( GetTokenEx( SEP_LCURLY, ctrl, resetpoint, &bfilereset) ) {
        ...
        if(resetpoint && bfilereset)
            return( TRUE );
        if( GetTokenEx( (CmdFlags & CF_MSLINK) ? SEP_COLON : SEP_NO, ctrl, resetpoint, &bfilereset) == FALSE )
            return( FALSE );
        do {
            if(resetpoint && bfilereset)
                return( TRUE );
...
/* TRUE) otherwise use return code from action routine in matching entry  */
{
    ...
    if ( *tcp == '-' || *tcp == '/' ) {
        ...
        while ( entry->keyword != NULL && ( entry->flags & CF_MSLINK ) ) {
        ...
        while ( entry->keyword != NULL ) {
            ...
            entry++; /* here if this is no match */
        }
    }
