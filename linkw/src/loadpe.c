...
/************************************/
{
    seg_leader			*idata;
#if 0 /* JWLink: idata */
    //return( IDataGroup->linear + FmtData.base.x86 + IData.iat_off );
#else
    idata = FindSegment( Root, CoffIDataSegName );
    return( idata->group->linear + GetLeaderDelta( idata ) +
	   FmtData.base.x86 + IData.iat_off );
#endif
}
...
/***********************************************/
{
    offset off = FindIATSymAbsOff(sym) - IDataGroup->linear - FmtData.base.x86;
...
/************************************/
{
    ...
    xferbase = group->linear + GetLeaderDelta( XFerSegData->u.leader ) + FmtData.base.x86;
...
/* make a PE executable file */
{
    ...
    exe_head.image_base = FmtData.base.x86;
